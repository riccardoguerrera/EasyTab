<?php
	...
	function easytab_product_tabs_metabox($post) {
		...
		foreach ($wc_tabs as $wc_tab_key => $wc_tab_data) {
			...
			<div style="margin: 10px 0;">
				<label for="easytab-<?php echo esc_html(str_replace('_', '-', $wc_tab_key)); ?>-tab">
					<strong>
						<?php echo esc_html($wc_tab_data['title']); ?>
					</strong>
				</label>
				<br>
				<textarea
					name="easytab_<?php echo esc_html(str_replace('-', '_', $wc_tab_key)); ?>_tab"
					id="easytab-<?php echo esc_html(str_replace('_', '-', $wc_tab_key)); ?>-tab"
					rows="4"
					style="width: 100%;"><?php echo wp_kses_post($wc_tab); ?></textarea>
				<small>
					<i>
						<?php esc_html_e('Remove the text to regenerate the content!', EASYTAB_DOMAIN); ?>
					</i>
				</small>
			</div>
	...
	function easytab_product_tabs_save_metabox($post_id, $post, $update) {
		...
		foreach ($wc_tabs as $wc_tab_key => $wc_tab_data) {
			$tab = !empty($_POST['easytab_' . str_replace('-', '_', $wc_tab_key) . '_tab']) ? wp_kses_post(wp_unslash($_POST['easytab_' . str_replace('-', '_', $wc_tab_key) . '_tab'])) : null;
